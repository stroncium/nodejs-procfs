...
module.exports = {
	...
	processStatus: asserter(Joi.object().keys({
		...
		namespaceThreadGroupIds: Joi.array().items(Joi.number().integer()).min(0),
		namespaceSessionIds: Joi.array().items(Joi.number().integer()).min(0),
		namespaceProcessGroupIds: Joi.array().items(Joi.number().integer()).min(0),
		namespacePids: Joi.array().items(Joi.number().integer()).min(0),
		name: Joi.string().allow('').required(),
	...
	processFdinfo: asserter(Joi.object().keys({
		...
		eventCounter: Joi.number().integer(),
